...
# 그렇지 않으면, 개발자(=나)가 회원들의 비밀번호를 볼 수 있으니까
import hashlib
...
def api_register():
    ...
    user_category_receive = int(request.form['user_category_give'])

    pw_hash = hashlib.sha256(pw_receive.encode('utf-8')).hexdigest()

    db.user.insert_one({'user_id': id_receive, 'password': pw_hash, 'name': name_receive,'contact':contact_receive,'user_category_id':user_category_receive})
...
def api_login():
    ...
    if result is not None:
        ...
        # exp에는 만료시간(5시간)을 넣어줍니다. 만료시간이 지나면, 시크릿키로 토큰을 풀 때 만료되었다고 에러가 납니다.
        payload = {
            'id': id_receive,
